...
def validate_review_coverage(review: dict, parsed: dict, domain_profile: dict, strict: bool) -> dict:
    ...
    endpoint_fields = ["inputs", "outputs", "research_objects", "concepts", "modules", "innovations", "results", "terminology"]
    endpoint_ids = {
        item.get("id")
...
def build_paper_review(parsed: dict, domain_profile: dict, mode: str = "vlm", model: str | None = None) -> tuple[dict, dict]:
    ...
    if mode == "vlm" and vlm_credentials_available():
        ...
        try:
            raw = call_vlm_json(prompt, [], model=resolved, timeout=300, retries=1)

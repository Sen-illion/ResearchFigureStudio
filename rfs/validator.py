...
def _valid_bbox_percent(bbox: object) -> bool:
...
def _validate_card_item(card: dict, label: str, errors: list[str]) -> None:
    card_id = card.get("id")
    if not str(card_id or "").strip():
        errors.append(f"{label} missing id")
    if not _valid_bbox_percent(card.get("bbox_percent")):
        errors.append(f"{label} {card_id} has invalid bbox_percent")
    if str(card.get("editable_in", "")).lower() != "pptx":
        errors.append(f"{label} {card_id} must be editable in PPTX")
    if card.get("asset_id") or card.get("path") or card.get("reference_crop_path"):
        errors.append(f"{label} {card_id} must not be an image asset")
    if str(card.get("semantic_role") or "") not in {"subcard_frame", "outer_group_boundary", "metric_badge", "legend_frame"}:
        errors.append(f"{label} {card_id} has invalid semantic_role")
    if str(card.get("shape_kind") or "") not in {"rounded_rect", "rect"}:
        errors.append(f"{label} {card_id} has invalid shape_kind")
    if not _valid_hex_color(card.get("stroke_color")):
        errors.append(f"{label} {card_id} has invalid stroke_color")
    if card.get("fill_color") is not None and not _valid_hex_color(card.get("fill_color")):
        errors.append(f"{label} {card_id} has invalid fill_color")
    try:
        if float(card.get("stroke_width_pt", 0)) <= 0:
            errors.append(f"{label} {card_id} stroke_width_pt must be positive")
    except Exception:
        errors.append(f"{label} {card_id} stroke_width_pt must be numeric")
    if str(card.get("dash_style") or "") not in {"solid", "dash", "dashed", "dotted"}:
        errors.append(f"{label} {card_id} has invalid dash_style")
    try:
        transparency = float(card.get("fill_transparency", 1.0))
        if transparency < 0 or transparency > 1:
            errors.append(f"{label} {card_id} fill_transparency must be between 0 and 1")
    except Exception:
        errors.append(f"{label} {card_id} fill_transparency must be numeric")
...
def validate_output(out_dir: str | Path) -> dict:
    ...
    try:
        ...
        for card in layout_plan.get("cards", []) if isinstance(layout_plan.get("cards", []), list) else []:
            if isinstance(card, dict):
                _validate_card_item(card, "layout_plan card", errors)
    except Exception as exc:
        errors.append(f"Invalid layout_plan.json: {exc}")

    try:
        program = json.loads((root / "figure_program.json").read_text(encoding="utf-8"))
        for key in ["canvas", "panels", "cards", "slots", "assets", "labels", "arrows", "groups", "export_targets"]:
            if key not in program:
                errors.append(f"figure_program.json missing {key}")
        slots = program.get("slots", [])
        card_ids = {str(card.get("id")) for card in program.get("cards", []) if isinstance(card, dict) and card.get("id")}
        if len(slots) < 25:
        ...
        for asset in program.get("assets", []):
            ...
            if str(asset.get("id") or "") in card_ids or str(asset.get("slot_id") or "") in card_ids:
                errors.append(f"Asset {asset.get('id')} incorrectly references a card frame")
        ...
        for card in program.get("cards", []) if isinstance(program.get("cards", []), list) else []:
            if isinstance(card, dict):
                _validate_card_item(card, "figure_program card", errors)

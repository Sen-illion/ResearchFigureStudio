...
ResearchFigureStudio 是一个 **PPTX-first 的科研框架图生成流水线**。主线 `make-framework` 读取论文和参考图，把参考图拆成许多局部图像槽，生成 25-50 个非箭头图像资产，再用可编辑的 PowerPoint 图层组合成科研框架图；`rebuild-editable` 则从单张参考图重建可编辑 PPT 组合。
...
- `coevolve-image` 是 whole-image creator/judge 迭代基线，服务于生成前置候选图；不能替代最终 PPTX 可编辑图层交付。
...
- 可选 OCR 依赖：`paddleocr`、`easyocr`，通过 `python -m pip install -e ".[ocr]"` 安装。
- Windows + 本机 PowerPoint 是最佳导出环境，因为 PDF/PNG 导出依赖 PowerPoint COM 和 PyMuPDF。

## 目录结构

- `rfs/`：主 Python 包。
- `rfs/cli.py`：命令行入口，提供 `doctor`、`make-framework`、`rebuild-editable`、`rebuild-editable-eval`、`coevolve-image`、`coevolution-report`、`presentations-qa`、`validate`。
...
- `rfs/editable_rebuild.py`：上游新增的 reference-only 可编辑 PPT rebuild 流水线，目标是从参考图重建可编辑 PowerPoint 组合。
- `rfs/rebuild_design_planner.py`：rebuild-editable 的全局参考图理解阶段，先生成 narrative、layer plan、generation policy 和 flow graph，再指导后续布局、箭头、语义和资产策略。
- `rfs/control_localizer.py`、`rfs/layout_planner.py`、`rfs/layout_semantic_planner.py`：rebuild-editable 使用的控件定位、布局规划和语义规划模块，支持 heuristic/VLM/hybrid，并可消费全局 design plan/flow graph。
- `rfs/arrow_router.py`：参考优先的箭头样式、路由、fallback 和 QA 报告。
- `rfs/text_layer.py`、`rfs/reference_text_extractor.py`、`rfs/text_role_classifier.py`：OCR/heuristic/VLM 文本抽取、角色分类和 PPT 可编辑文本层。
- `rfs/rebuild_vlm_adapters.py`、`rfs/rebuild_vlm_validation.py`、`rfs/rebuild_eval.py`、`rfs/vlm_client.py`：rebuild 工作流的 VLM 调用、输出验证和 heuristic-vs-VLM 评估支持。
- `rfs/presentations_qa.py`：可选 Presentations 插件 QA，导入/渲染/检查 PPTX，但不修改交付文件。
- `rfs/coevolution/`：whole-image Creator/Judge co-evolution 实验基线和指标汇总。
...
- `tests/`：覆盖 validator、rebuild、arrow router、OCR text layer、Presentations QA、coevolution 等关键行为。
- `docs/architecture.md`、`docs/workflow.md`、`docs/rebuild-editable.md`、`docs/coevolution.md`：架构、流程和新增实验入口说明。
...
figure_program.json -> editable text layer -> arrow routing/style reports ->
slot_visual_spec.json -> reference_slot_prompt_brief.json ->
...
- `rebuild-editable` 可用 crop/api/placeholder 生成槽资产，但仍要保持文字、箭头、连接器和控件在 PPT 层可编辑。
- `rebuild-editable` 的全局 VLM 规划只输出结构化合同，不直接写 PPT 代码；最终坐标、文字、箭头和层级仍由本地合同清洗、OCR/ownership、arrow router 和 PPT compiler 落地。
...
上游新增 reference-only 可编辑重建命令：

```powershell
rfs rebuild-editable `
  --reference "C:\path\reference.png" `
  --out "output\rebuild_check" `
...
rfs rebuild-editable-eval `
  --reference "C:\path\reference.png" `
  --out "output\rebuild_eval" `
  --asset-mode crop `
  --text-mode ocr `
  --json
```

可选 whole-image creator/judge 迭代基线：

```powershell
rfs coevolve-image `
  --ground-truth "C:\path\ground_truth.json" `
...
```powershell
rfs presentations-qa --out "output\offline_check" --json
```
...
- `MODEL_VLM`
- `RFS_LOCATOR_MODEL`
- `RFS_CONTROL_LOCALIZER_MODEL`
- `RFS_REBUILD_LAYOUT_MODEL`
- `RFS_REBUILD_DESIGN_MODEL`
- `RFS_REBUILD_CONTROL_MODEL`
- `RFS_REBUILD_SEMANTIC_MODEL`
- `RFS_PROMPT_PLANNER_MODEL`
- `RFS_TEXT_ROLE_MODEL`
- `RFS_CRITIC_MODEL`
- `RFS_ONLINE_JUDGE_MODEL`
- `RFS_FROZEN_JUDGE_MODEL`
- `RFS_IMAGE_MODEL`
- `RFS_IMAGE_EDIT_URL`
- `IMAGE_MODEL`
...
`rebuild-editable` 还会写出 `rebuild_result.json`、`reference_logic_plan.json`、`reference_layer_plan.json`、`reference_generation_plan.json`、`reference_flow_graph.json`、`ocr_refinement_report.json`、`ocr_text_corrections.json`、`asset_generation_specs.json`、`asset_generation_report.json`、`asset_economy_report.json`、`asset_ratio_fit_report.json`、`slot_semantic_report.json`、`rebuild_vlm_validation_report.json`、`reference_geometry_overlay.png`、`reference_controls_overlay.png` 等重建专用 artifact。

`coevolve-image` 会写出 `approved_image.png`、`run_report.json`、`rounds/round_*/round_summary.json`、`preference_pairs.jsonl`、`repair_trajectories.jsonl`、`critique_outcomes.jsonl`、`reproduction_metrics.json` 和 `reproduction_metrics.md`。
...
- OCR 和文本角色分类失败时可以 heuristic fallback，但必须在报告中明示，不能伪装成高置信 OCR/VLM 结果。
...
运行项目时优先使用虚拟环境；仓库根目录 `.env` 里的 API 变量需要手动注入当前 shell，不要依赖代码自动读取密钥文件。
...
- Presentations QA 是检查层，不是编译权威；不要让它反向改写 RFS 的 PPTX 合同。



## 约定
1. 每次进行修改后都要提交一次commit，提交简介简单概括本次修改内容，并且给出commit ID，目的是保证修改不满意可退回到上一次修改。
2. 每次拉取新的代码先更新 `AGENTS.md` 的快速理解项目部分。
3. 运行项目时在虚拟环境里运行，API在.env文件中，需要手动注入。
4. 每次拉取或合并代码时，如果出现冲突，不要自行保守合并；先列出冲突文件、双方差异和推荐选项，询问 Sen-illion 后再决定保留哪个实现或如何组合。
5. 如果有同时修改的内容，一次commit只要提交本次自己修改的内容，不要碰其他的修改。
